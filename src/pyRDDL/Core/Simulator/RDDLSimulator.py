...
class RDDLSimulator:
    ...
    def set(self, states: Args) -> Union[Dict[str, None], Args]:
        '''sets the state variables to their assigned values.
        
        :param states: a dict mapping current states fluent to their values
        '''
        rddl = self.rddl
        subs = self.subs = self.init_values.copy()
        subs.update(states)
